		...
		if (classState == false){
			...
			if (cacheData == null){
				switch(currentCategoryLevel){
					...
					//Not caching at initial level because the whole data will be put in cache which will increase the cache size
					//considerably
	 ...
	 if(isTreeExpandIconClicked )
		...
		var isIdExist = $('#innerID', '#'+assignedOrgNodeIds).length;
			if(isIdExist > 0){
				...
				console.log("OrganizationNodes"  + organizationNodes);
				leafParentOrgNodeId = "";
				...
				$.ajax(
						{
								...
								success:	function(data, textStatus, XMLHttpRequest){	
												...
												if(data.isEdit){
												console.log("IS Edit tree" + data.isEdit);
												...
												console.log("Add tree" + data.organizationDetail.parentOrgNodeId +"," + data.organizationDetail.orgNodeName+"," +data.organizationDetail.orgNodeId+"," +data.organizationDetail.categoryLevel);
